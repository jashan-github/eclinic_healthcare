...
from app.api.rest import chat as rest_chat
...
async def lifespan(app: FastAPI):
    ...
    # Small delay to allow network to initialize
    import asyncio
    await asyncio.sleep(1)
    
    # Try to connect to Redis, but don't fail startup if it's not available
    # Redis will connect lazily when needed (in publish_message/subscribe_to_room)
    try:
        await redis_service.connect()
        logger.info("Chat Service started successfully with Redis connection")
    except Exception as e:
        logger.warning(f"Redis not available at startup: {e}. Will retry when needed.")
        # Don't raise - allow service to start without Redis
        # Redis connection will be attempted when actually needed
...
app = FastAPI(
    ...
    description="Real-time chat service for healthcare platform",
        ...
        openapi_schema["servers"] = [
            {
                ...
                "description": "Chat Service API (via reverse proxy)"
            }
        ]
...
# CORS middleware - must allow WebSocket connections
app.add_middleware(
    CORSMiddleware,
...
# WebSocket test endpoint (no auth required for testing)
@app.get("/ws/test")
async def websocket_test():
    """Test endpoint to verify WebSocket routing"""
    return {
        "status": "ok",
        "message": "WebSocket endpoint is accessible",
        "routes": [str(r.path) for r in app.routes if hasattr(r, 'path') and 'ws' in str(r.path)]
    }
...
# Include routers
app.include_router(rest_chat.router, prefix="/api")
